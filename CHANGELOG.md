...
- Added a unique "Ion ID" column to ion and ion evidence tables from all supported tools (FragPipe, MaxQuant, Spectronaut), which is generated by concatenating "Modified sequence", "Charge", and optionally "Compensation voltage".
  - The "Ion ID" column is meant to be used as `id_column` when creating a `Qtable` instance.

### Dependencies
- Made the R interface optional by moving `rpy2` to optional dependencies
  - Install with `pip install msreport[R]` to enable statistical analysis with LIMMA
- Added optional `test` dependencies for running tests with GitHub actions
- Dropped Python 3.9 support, now requires Python 3.10+

### Internal
- Added automated testing via GitHub Actions
- Improved README structure, documentation of features, and installation instructions
- Added release dates to CHANGELOG entries
