...
def missing_values_per_category(
        method_tables: dict[str, pd.DataFrame], experiments: str,
        categories: list[str] = None, method_colors: dict[str, str] = None):
    ...
    for category in categories:
        ...
        fig, axes = missing_values(
            tables, experiments, method_colors=method_colors
        )
...
def missing_values(
        method_tables: dict[str, pd.DataFrame], experiments,
    ...
    for exp_num, exp in enumerate(experiments):
        for method_num, method in enumerate(method_tables):
            ...
            ax.bar(
                x, y, width=barwidth, label=method,
...
def ratio_dist_per_category(
        ...
        expected_ratios: dict[str, float] = None, max_missing: int = 10,
        method_colors: dict[str, str] = None):
...
def precision_tp(
    ...
    for method_num, (method, table) in enumerate(method_tables.items()):
        ...
        ax.plot(
            curve_values['TP'], curve_values['precision'],
            color=method_colors[method], label=method
        )
        for pvalue, precision, tp in zip(curve_values['pvalue'],
