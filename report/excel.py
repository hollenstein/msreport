...
class Datasheet:
    ...
    def add_data(self, table: pd.DataFrame) -> None:
        ...
        if not self._samples and extraction_tag is not None:
            self._samples = extract_samples_with_column_tag(self._table, extraction_tag)
    ...
    def _prepare_comparison_group(self, name, config):
        ...
        for column_tag in config["columns"]:
            for column in helper.find_columns(self._table, column_tag):
        ...
        for comparison_group in comparison_groups:
            ...
            matched = helper.find_columns(self._table, comparison_group)
            columns = []
            for column_tag in config["columns"]:
    ...
    def _find_sample_group_columns(self, tag: str) -> [list[str], list[str]]:
        ...
        """
        matched_columns = helper.find_columns(self._table, tag, must_be_substring=False)
        non_sample_columns = []
        sample_columns = []
